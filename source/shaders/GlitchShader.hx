...
{
	public var glitchSpeedMult:Float = 1;
	
	public var squareAmount(default, set):Float = 0;
	
	function set_squareAmount(v:Float)
	{
		return (prob.value[0] = squareAmount = v);
	}
	
	public var glowAmount(default, set):Float = 0;
	
	function set_glowAmount(v:Float)
	{
		return (intensityChromatic.value[0] = glowAmount = v);
	}
...
')
	public function new()
	{
		...
		this.prob.value = [0, 0];
		this.intensityChromatic.value = [0, 0];
	...
	{
		this.time.value[0] += elapsed * glitchSpeedMult;
	}
	
	public function toString()
	{
		var buff = new StringBuf();
		buff.add('speedMult: ' + glitchSpeedMult);
		buff.add(' : squareAmount: ' + squareAmount);
		buff.add(' : glowAmount: ' + glowAmount);
