...
var nullGlitchBg:FlxSprite; // declared early so it compiles
...
var view:View3D; // 3D rendering viewport
...
{
	// ensure the 3D view exists before any assets are loaded or manipulated
	view = new View3D();
	view.width = FlxG.width;
	view.height = FlxG.height;
	view.backgroundColor = 0x000000;
	// anti-aliasing helps with jagged edges, but it can be expensive
	view.antiAlias = 2;
	// by default the view will be added to the OpenFL display list rather than
	// the FlxG.camera system, so make it a child of the state so it'll render
	FlxG.state.addChild(view);
...
function onUpdate(elapsed:Float)
{
	if (loaded3d && view != null && view.camera != null)
	{
		...
		// render the 3D image after the camera has been updated
		view.render();
	}
...
{
	if (view == null) return; // nothing to toggle yet
	view.visible = active;
